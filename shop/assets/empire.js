  ...
  createClass(StickyHeader, [{
    ...
    value: function openNavigation() {
      ...
      this.$el.addClass(this.navOpenClass).one('trend', function () {
        _this3.$body.trigger('position-banners');
      });
    }
  }, {
    key: 'navVisibility',
    ...
    value: function _bindEvents() {
      ...
      this.$toggle.on('click.sticky-header', function (event) {
        ...
        if (layout.isBreakpoint('L')) _this4._slideToggle();
      });
    ...
    value: function _shrink() {
      ...
      this.$el.revealer('show').one('revealer-animating.sticky-header', function () {
        _this6.$body.addClass(_this6.scrolledClass).one('trend', function () {
    ...
    value: function _expand() {
      ...
      this.$el.revealer('hide').one('revealer-animating.sticky-header', function () {
        _this7.$body.removeClass(_this7.scrolledClass).one('trend', function () {
          _this7.$body.trigger('position-banners');
        });
      }).one('revealer-hide.sticky-header', function () {
        _this7.$toggle.removeClass('active');
      });
    }
  }, {
    key: '_headerChecker',
...
var LiveSearch = function () {
  function LiveSearch($els, settings) {
    ...
    classCallCheck(this, LiveSearch);
    ...
    this.events = [this.$button.on('click.live-search', function (event) {
      event.preventDefault();
      _this._search(event);
    }), this.$input.on('keyup.live-search', justDebounce$1(this._search, 250)), this.$input.on('focus.live-search', this._searchFocused), this.$takeOverButton.on('click.live-search', this._toggleTakeOver)];
  }

  createClass(LiveSearch, [{
    key: 'unload',
    value: function unload() {
      this.events.forEach(function ($el) {
    ...
    value: function _searchFocused() {
      ...
      this.closeEvents.concat([this.$header.on('focus.live-search', 'a, button', this._documentFocus), this.$document.on('touchstart.live-search', this._documentFocus), this.$document.on('click.live-search', this._documentFocus)]);
...
var StaticSectionHeader = function () {
  ...
  createClass(StaticSectionHeader, [{
    ...
    value: function onSectionBlockSelect(block) {
      ...
      setTimeout(function () {
        if (_this2.settings.sticky_header) {
          _this2.stickyHeader.slideToggle(true);
        }
    ...
    value: function onSectionBlockDeselect() {
      if (this.$menuToggle.hasClass('active') && layout.isBreakpoint('L') && this.settings.sticky_header) {
...
var Pixelzoom = function () {
  function Pixelzoom(container, options) {
    classCallCheck$3(this, Pixelzoom);
...
var ProductZoom = function () {
  function ProductZoom(options) {
    classCallCheck(this, ProductZoom);
  ...
  createClass(ProductZoom, [{
    ...
    value: function enableZoom(index) {
      ...
      var $newSlide = this.$slider.find('[data-product-image-index=' + this.index + '] .product-galley--image-background');
      var inObject = !this.imageZoom[this.index];
      var slideExists = this.$gallery.find('[data-product-image-index=' + this.index + '][data-product-image-zoom]').length;
      ...
      if (inObject && slideExists) {
        ...
        $newSlide.on('pixelzoom:touch', function () {
          ...
          if (!layout.isBreakpoint('XS') && (imageWidth / $newSlide.width() >= 2 || imageHeight / $newSlide.height() >= 2)) {
            _this2._toggleClass(jquery$1('body'), 'pixelzoom--active');
          }
        ...
        $newSlide.on('pixelzoom:release', function () {
          _this2._toggleClass(jquery$1('body'), 'pixelzoom--active', true);
    ...
    value: function _bindEvents() {
      ...
      this.$slider.on('dragStart.flickity', function (event) {
        _this3._preventClick();
      });

      this.$slider.on('settle.flickity', function (event) {
        _this3._resolveClick();
      });

      this.$slider.on('click.product-zoom', function () {
        _this3._openModal();
      });
    ...
    value: function _onOpen() {
      ...
      setTimeout(function () {
        _this4.zoomOverlay = new Pixelzoom(document.querySelector('[data-modal-content]'), {
          imageSource: $newSlide[0].dataset.image,
          naturalPan: true,
          dragPan: true,
        ...
        jquery$1('[data-modal-content]').on('pixelzoom:loaded', function () {
          jquery$1('[data-modal-content]').addClass('loaded');
        });

        jquery$1('[data-modal-content]').on('pixelzoom:touch', function () {
          _this4._toggleClass(jquery$1('body'), 'pixelzoom--active');
        });

        jquery$1('[data-modal-content]').on('pixelzoom:release', function () {
          _this4._toggleClass(jquery$1('body'), 'pixelzoom--active', true);
        });
      }, 500);
    }
  }, {
    key: '_onClose',
    value: function _onClose() {
      jquery$1('[data-modal-content]').removeClass('loaded');
...
var ProductGridItem = function () {
  ...
  createClass(ProductGridItem, [{
    ...
    value: function _objectFitPolyfill() {
      ...
      var $figure = jquery$1('[data-product-item-image]', this.$el);
      var featuredSrc = jquery$1('img:not(.productitem--image-alternate)', $figure).attr('src');
      var alternateSrc = jquery$1('.productitem--image-alternate', $figure).attr('src');
...
var StaticSlideshow = function () {
  ...
  createClass(StaticSlideshow, [{
    ...
    value: function _initSlider() {
      ...
      this.$firstSlideImage = jquery$1(this.slides, this.carousel).find('img[data-rimg]').first();

      this.$firstSlideImage.one('rimg:load', function () {
        _this.flickity = new js$1(_this.carousel, _this.flickityOptions);
        _this._bindEvents();
      });
    }
  }, {
...
var DynamicSearch = function () {
  function DynamicSearch(section) {
    ...
    classCallCheck(this, DynamicSearch);
  ...
  function PxsMap(section) {
    ...
    if (this.apiKey) {
      if (window.googleMaps === undefined) {
        script$1('https://maps.googleapis.com/maps/api/js?key=' + this.apiKey, function () {
